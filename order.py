    ...
    def execute(self, book):
        ...
        for sym in book.symbol:
            ...
            if not pd.isnull(book.bar.data.ix[sym, -1, 'Close']):
                ...
                actual_execute_amount = int( (book.book.ix[sym, book.ind, 'TargetPos']-book.book.ix[sym, book.ind, 'PrePos'])
                                            * (1 if random.uniform > self.missing_rate else self.missing_rate))
                book.book.ix[sym, book.ind, 'Pos'] = book.book.ix[sym, book.ind, 'PrePos'] + actual_execute_amount
                actual_execute_price = (1 + math.copysign(self.slippage_rate, actual_execute_amount)/100) \
                                       * float(book.bar.data.ix[sym, book.ind, 'Close'])
                book.book.ix[sym, book.ind, 'mmk'] = actual_execute_price
                book.book.ix['Cash', book.ind, 'Pos'] -= actual_execute_amount * actual_execute_price
